...
3. **Video Rendering** — Bundles and renders a Remotion composition (`CodingTutorial`) into an MP4 (1920x1080 @ 30fps, H.264) via Node.js subprocess (with `--max-old-space-size=1024`). Chromium is configured with `enableMultiProcessOnLinux: false`, `--disable-dev-shm-usage`, and `--disable-gpu` for stability on Linux. Concurrency is set to 1. Stale `chrome-headless-shell` processes are killed before and after each render to prevent memory leaks across runs. Audio durations are measured by Remotion's `calculateMetadata` (using `@remotion/media-utils`) for accurate frame-level sync.
